...
import com.blogic.androidgames.framework.gl.FPSCounter;
...
import com.blogic.androidgames.greeracer.Gear;
import com.blogic.androidgames.greeracer.Race;
import com.blogic.androidgames.greeracer.RaceRenderer;
import com.blogic.androidgames.greeracer.Settings;
import com.blogic.androidgames.greeracer.Ship;
import com.blogic.androidgames.greeracer.Race.RaceListener;
...
public class ScreenGame extends GLScreen {
	...
	private void presentRace() {
		...
		gl.glColor4f(0.20f, 0.20f, 0.20f, 0.80f);
		batcher.beginBatch(Assets.items);
		batcher.drawSprite(camWidth/2f,  camHeight - 1f * gaugeGap, 700f, 38f, Assets.pauseRegion);
		batcher.endBatch();

		gl.glColor4f(0.4f, 0.5f, 0.5f, 1f);
		batcher.beginBatch(Assets.items);
		batcher.drawSprite(camWidth/2f,  camHeight - 1f * gaugeGap, distGaugeWidth+3, distGaugeHeight+3, Assets.pauseRegion);
		batcher.drawSprite(camWidth/2f,  camHeight - 2f * gaugeGap, distGaugeWidth+3, distGaugeHeight+3, Assets.pauseRegion);
		batcher.endBatch();
		
		gl.glColor4f(0, 0, 0, 1f);
		batcher.beginBatch(Assets.items);
		batcher.drawSprite(camWidth/2f,  camHeight - 1f * gaugeGap, distGaugeWidth+2, distGaugeHeight+2, Assets.pauseRegion);
		batcher.drawSprite(camWidth/2f,  camHeight - 2f * gaugeGap, distGaugeWidth+2, distGaugeHeight+2, Assets.pauseRegion);
		batcher.endBatch();
	...
	private void presentNumbers() {
		...
		if (!isRecording && race.carRival.getFinishTime() > 0) {
...
//			Assets.font.drawText(batcher, Math.round(race.carRival.timeToFinishLine  * 100f) / 100f + "", 190.0f, yOffset - 20, 0.5f);
			Assets.font.drawText(batcher, Math.round(race.carRival.getFinishTime()  * 1000000f) / 1000000f + "", 231, yOffset - 20, 1.0f);
//			Assets.font.drawText(batcher, Math.round(race.carRival.getTimeToVelocity(race.carRival.shiftAtVelocity)  * 1000f) / 1000f + "", camHeight, yOffset - 20, 0.5f);
		...
		if (race.carPlayer.getFinishTime() > 0) {
...
//			Assets.font.drawText(batcher, Math.round(race.carPlayer.timeToFinishLine * 100f) / 100f + "", 190.0f, yOffset - 40, 0.5f);
			Assets.font.drawText(batcher, Math.round(race.carPlayer.getFinishTime() * 1000000f) / 1000000f + "", 231, yOffset - 40, 1.0f);
